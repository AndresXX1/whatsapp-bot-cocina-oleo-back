...
let collectionRespuestas; // Variable global para almacenar la colección de respuestas

// Función para conectar a MongoDB una vez
async function connectDB() {
    try {
        if (!collectionRespuestas) {
            await clientDB.connect();
            console.log("Conectado a MongoDB");
            const db = clientDB.db(); 
            collectionRespuestas = db.collection('respuestas');
        }
        return collectionRespuestas;
    } catch (error) {
        console.error('Error conectando a MongoDB:', error);
    }
}

// Función para inicializar las respuestas si la colección está vacía
...
(async () => {
    await initializeResponses(); // Inicializar respuestas solo una vez
    client.initialize();
})();
